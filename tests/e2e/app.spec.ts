...
test('無線機詳細とナミゲート効果パラメータで推定値が変わる', async ({ page }) => {
  ...
  await expect(page.getByText(/適用範囲外/)).toBeVisible()
...
test('CSV実測点を取り込み校正とレポート作成ができる', async ({ context, page }) => {
  ...
  await expect(page.getByText('校正候補を入力条件へ反映しました')).toBeVisible()

  await page.getByRole('button', { name: '実証レポートをコピー' }).click()
  await expect(page.getByText('実証レポートをコピーしました')).toBeVisible()
  ...
  await page.getByPlaceholder('Low-E窓 ナミゲート20cm 室内8m').fill('E2E実証ケース')
  await page.getByRole('button', { name: '保存', exact: true }).click()
  await expect(page.getByText('試験ケースを保存しました')).toBeVisible()
